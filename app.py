...
def display_recommendations_with_individual_maps(path, tags_list, route_idx=None):
    ...
    unique_key = f"multiselect_{'_'.join(path)}_{route_idx if route_idx is not None else 'single'}"
    if st.session_state['recommendations_done']:
        ...
        if st.button(
    "🔍 Include Selected Places in Journey",
    key=f"include_selected_places_{'_'.join(path)}_{route_idx if route_idx is not None else 'single'}"
):
            if selected_places:
                selected_place_objects = []
                for city in path:
                    recs = recommend_places(df, city, tags_list)
                    for rec in recs:
                        if rec['pName'] in selected_places:
                            selected_place_objects.append(rec)
                
                # Remove duplicates while preserving order
                seen = set()
                unique_selected = []
                for p in selected_place_objects:
                    if p['pName'] not in seen:
                        seen.add(p['pName'])
                        unique_selected.append(p)
                
                journey_map = create_map_with_selected_places(unique_selected, city_coordinates, path)
                map_html = f'journey_map_{route_idx if route_idx is not None else "single"}.html'
                journey_map.save(map_html)
                st.components.v1.html(open(map_html, 'r').read(), height=400)
            else:
        ...
        with st.spinner("Calculating optimal paths..."):
            ...
            if all_paths:
                ...
                with tab2:
                    ...
                    display_recommendations_with_individual_maps(path1, tags_list, route_idx=1)
                ...
                with tab3:
                    ...
                    route_map.save(map_html)
                    st.components.v1.html(open(map_html, 'r').read(), height=400)
                    display_recommendations_with_individual_maps(path2, tags_list, route_idx=2)
                ...
                with tab4:
                    ...
                    route_map.save(map_html)
                    st.components.v1.html(open(map_html, 'r').read(), height=400)
                    display_recommendations_with_individual_maps(path3, tags_list, route_idx=3)
                ...
                with tab5:
                    ...
                    route_map.save(map_html)
                    st.components.v1.html(open(map_html, 'r').read(), height=400)
                    display_recommendations_with_individual_maps(best_path, tags_list, route_idx='best')
